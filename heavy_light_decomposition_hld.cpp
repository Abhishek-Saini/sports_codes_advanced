...
int main(){
    ...
    while(T--) {
        ...
        while(q--) {
            ...
            if(a == 0) {
                A[b] = 1 - A[b];
                SegTree.update(PosInFlatArray[b], A[b]);
            }
            else {
                // Here LCA calculation can also be done often
                auto res = query(b, 1, SegTree);
                if(res.first == -1) {
                    printf("-1\n");
                    continue;
                }
                int low = PosInFlatArray[res.first], high = PosInFlatArray[res.second];
                int ans = SegTree.findFirstOne(low, high);
                printf("%d\n",ReversePosInFlatArray[ans]);
            }
        }
    }
    return 0;
